            ...
            size_t char_offset, size_t *entry_offset_byte_rtn )
{
    ...
    */
    ...
    while (idx >= buffer->in_offs && !reached_end)
    {	
    	if (buffer->entry[idx].size <= (char_offset - count))
    	{
    		count += buffer->entry[idx].size;
    		++idx;
    		if (AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED == idx)
    		{
    			reached_end = true;
